...
from src.python.networking.serialization import LaserTagJSONDecoder


def handler(message_type):
    def handler_decorator(func):
        @functools.wraps(func)
        def handler_wrapper(self, session, dictionary, timestamp):
            try:
                ...
                func(self, session, msg, timestamp)
            except:
...
class MessageHandler:
    ...
    def handle_message(self, session, json_string, timestamp):
        dictionary = json.loads(json_string)
        if "__id" in dictionary:
            id = dictionary["__id"]
            if id in self._message_handlers:
                self._message_handlers[id](session = session, dictionary = dictionary, timestamp = timestamp)
            else:
